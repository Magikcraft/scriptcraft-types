...
interface Java{
   type(type: 'org.bukkit.event.inventory.InventoryType.SlotType'): InventoryTypeSlotTypes
}
...
interface Java{
   type(type: 'org.bukkit.inventory.InventoryView.Property'): InventoryViewPropertys
}
...
interface MerchantRecipeConstructor {
   ...
   new(result: ItemStack, maxUses: number, experienceReward: boolean): MerchantRecipe
   new(result: ItemStack, maxUses: number, experienceReward: boolean, villagerExperience: number, priceMultiplier: number): MerchantRecipe
}
...
interface Java{
   type(type: 'org.bukkit.potion.PotionEffectType'):  PotionEffectTypeConstructor
}
...
interface  PotionEffectConstructor {
      ...
      new(type: PotionEffectType, duration: number, amplifier: number):  PotionEffect
      new(type: PotionEffectType, duration: number, amplifier: number, ambient: boolean):  PotionEffect
      new(type: PotionEffectType, duration: number, amplifier: number, ambient: boolean, particles: boolean):  PotionEffect
      new(type: PotionEffectType, duration: number, amplifier: number, ambient: boolean, particles: boolean, icon: boolean):  PotionEffect
}
...
interface  RayTraceResultConstructor {
   ...
   new(hitPosition: Vector, hitBlock: Block, hitBlockFace: BlockFace):  RayTraceResult
   new(hitPosition: Vector, hitEntity: Entity):  RayTraceResult
   new(hitPosition: Vector, hitEntity: Entity, hitBlockFace: BlockFace):  RayTraceResult
}
...
interface LivingEntity extends Attributable, Damageable, ProjectileSource {
   ...
   getLastTwoTargetBlocks​(transparent: Material[], maxDistance: number): Block[]
...
interface  ChunkGenerator extends Object {
   ...
   generateChunkData​(world: World, random: Random, x: number, z: number, biome: ChunkGeneratorBiomeGrid): ChunkGeneratorChunkData
...
interface  WorldCreator extends Object {
   ...
   getGeneratorForName​(world: string, name: string, output: CommandSender): ChunkGenerator
...
interface Java{
   type(type: 'org.bukkit.conversations.ConversationContext'):  ConversationContextConstructor
}

interface  ConversationContextConstructor {
   new(plugin: Plugin, forWhom: Conversable, initialSessionData: {[index: string]: Object}):  ConversationContext
}
...
interface Java {
   type(type: 'org.bukkit.conversations.ConversationAbandonedEvent'): ConversationAbandonedEventConstructor
}

interface  ConversationAbandonedEventConstructor {
   new(conversation: Conversation):  ConversationAbandonedEvent
   new(conversation: Conversation, canceller: ConversationCanceller): ConversationAbandonedEvent
}
...
interface  ConversationConstructor {
   new(plugin: Plugin, forWhom: Conversable, firstPrompt: Prompt):  Conversation
   new(plugin: Plugin, forWhom: Conversable, firstPrompt: Prompt, initialSessionData: {[index: string]: Object}):  Conversation
}
...
interface Java{
   type(type: 'org.bukkit.conversations.Conversation.ConversationState'): ConversationConversationStates
}
...
interface  CommandConstructor {
   ...
   new(name: string, description: string, usageMessage: string, aliases: string[]):  Command
}
...
interface CommandMap {
   ...
   /** Looks for the requested command and executes an appropriatetab-completer if found. */
   tabComplete​(sender: CommandSender, cmdLine: string, location: Location): string[]
}
...
interface  Command extends Object {
   ...
   /**   */
   broadcastCommandMessage​(source: CommandSender, message: string, sendToSource: boolean): void
   ...
   /** Executed on tab completion for this command, returning a list ofoptions the player can tab through. */
   tabComplete​(sender: CommandSender, alias: string, args: string[], location: Location): string[]
...
interface HelpMap {
   ...
   registerHelpTopicFactory​<T>(commandClass: new (...args: any[]) => T, factory: HelpTopicFactory<any>): void
}
...
interface ItemTagType<T,Z> {
   ...
   /** Deprecated.  */
   TAG_CONTAINER: ItemTagType<CustomItemTagContainer<any, any>,CustomItemTagContainer<any, any>>
...
interface ItemMeta extends Cloneable<ItemMeta> {
   ...
   setAttributeModifiers​(attributeModifiers: any /** com.google.common.collect.Multimap<Attribute,AttributeModifier> */): void
...
interface Java{
   type(type: 'org.bukkit.plugin.messaging.PluginMessageListenerRegistration'):  PluginMessageListenerRegistrationConstructor
}

interface  PluginMessageListenerRegistrationConstructor {
   new(messenger: Messenger, plugin: Plugin, channel: string, listener: PluginMessageListener):  PluginMessageListenerRegistration
}
...
interface Messenger {
   ...
   getIncomingChannelRegistrations​(channel: string): PluginMessageListenerRegistration[]
   /** Gets a set containing all the incoming plugin channel registrationsthat the specified plugin has. */
   getIncomingChannelRegistrations​(plugin: Plugin): PluginMessageListenerRegistration[]
   ...
   getIncomingChannelRegistrations​(plugin: Plugin, channel: string): PluginMessageListenerRegistration[]
...
interface TabCompleter {
   ...
   onTabComplete​(sender: CommandSender, command: Command, alias: string, args: string[]): string[]
}
...
interface Java{
   type(type: 'org.bukkit.plugin.RegisteredListener'):  RegisteredListenerConstructor
}
...
interface  RegisteredListenerConstructor {
   new(listener: Listener, executor: EventExecutor, priority: EventPriority, plugin: Plugin, ignoreCancelled: boolean):  RegisteredListener
}
...
interface Java{
   type(type: 'org.bukkit.plugin.PluginDescriptionFile'):  PluginDescriptionFileConstructor
}
...
interface PluginLoader {
   ...
   createRegisteredListeners​<T extends Event>(listener: Listener, plugin: Plugin): Map<new(...args) => T, RegisteredListener[]>
...
interface PluginManager {
   ...
   registerEvent​<T extends Event>(event: new (...args: any[]) => T, listener: Listener, priority: EventPriority, executor: EventExecutor, plugin: Plugin): void
   /** Registers the specified executor to the given event class */
   registerEvent​<T extends Event>​(event: new (...args: any[]) => T, listener: Listener, priority: EventPriority, executor: EventExecutor, plugin: Plugin, ignoreCancelled: boolean): void
   /** Registers all the events in the given listener class */
   registerEvents​(listener: Listener, plugin: Plugin): void
   /** Registers the specified plugin loader */
   registerInterface​<T extends PluginLoader>(loader: new (...args: any[]) => T): void
...
interface ChatColor {
   ...
   translateAlternateColorCodes​(altColorChar: string, textToTranslate: string): string
   /** Returns the enum constant of this type with the specified name. */
...
interface Server extends PluginMessageRecipient {
   ...
   /** Creates a new BlockData instance for the specified Material, withall properties initialized to unspecified defaults. */
   createBlockData​(material: Material, consumer: (BlockData) => void): BlockData
   ...
   getTag​<T extends Keyed>(registry: string, tag: NamespacedKey, clazz: new(...args) => T): Tag<T>
   /** Gets a all tags which have been defined within the server. */
   getTags​<T extends Keyed>(registry: string, clazz: new(...args) => T): Iterable<Tag<T>>
...
interface Entity extends Metadatable, CommandSender, Nameable, PersistentDataHolder {
   ...
   /** Teleports this entity to the target Entity. */
   teleport​(destination: Entity, cause: PlayerTeleportEventTeleportCause): boolean
   /** Teleports this entity to the given location. */
   teleport​(location: Location): boolean
   /** Teleports this entity to the given location. */
   teleport​(location: Location, cause: PlayerTeleportEventTeleportCause): boolean
}
...
interface  EntityDamageEventConstructor {
   ...
   new(damagee: Entity, cause: EntityDamageEventDamageCause, modifiers:Map<EntityDamageEventDamageModifier, number>, modifierFunctions: Map<EntityDamageEventDamageModifier, any>):  EntityDamageEvent
}
...
interface Java{
   type(type: 'org.bukkit.entity.AbstractArrow.PickupStatus'): AbstractArrowPickupStatuss
}
...
interface World extends PluginMessageRecipient, Metadatable {
   ...
   /** Deprecated.  */
   getEntitiesByClass​<T extends Entity>(...classes: (new(...args) => T)[]): T[]
   ...
   /** Returns a list of entities within a bounding box centered around aLocation. */
   getNearbyEntities​(location: Location, x: number, y: number, z: number, filter: (e: Entity) => boolean): Entity[]
   /** Returns a list of entities within the given bounding box. */
   getNearbyEntities​(boundingBox: BoundingBox): Entity[]
   /** Returns a list of entities within the given bounding box. */
   getNearbyEntities​(boundingBox: BoundingBox, filter: (e: Entity) => boolean): Entity[]
   ...
   rayTrace​(start: Location, direction: Vector, maxDistance: number, fluidCollisionMode: FluidCollisionMode, ignorePassableBlocks: boolean, raySize: number, filter: (e: Entity) => boolean): RayTraceResult
   ...
   /** Performs a ray trace that checks for entity collisions. */
   rayTraceEntities​(start: Location, direction: Vector, maxDistance: number, raySize: number, filter: (e: Entity) => boolean): RayTraceResult
   /** Performs a ray trace that checks for entity collisions. */
   rayTraceEntities​(start: Location, direction: Vector, maxDistance: number, filter: (e: Entity) => boolean): RayTraceResult
   ...
   spawn​<T extends Entity>(location: Location, clazz: new(...args) => T, fn: Consumer<T>): T
   ...
   spawnArrow​<T extends AbstractArrow>(location: Location, direction: Vector, speed: number, spread: number, clazz: new(...args) => T): T
